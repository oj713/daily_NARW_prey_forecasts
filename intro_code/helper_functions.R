...
recover_interval_dims <- function(stars_obj, true_offset = 1/12) {
  ...
  # Extract values of dimensions that 1) have no offset and 2) are numeric
  # We assume that any dims satisfying these criteria need fixing
  ...
  for (dim_name in names(dims_values)) {
    # Round values to the nearest (true_offset/2)th, dividing by 2 due to stars 'center' behavior
...
read_quantile_stars <- function(file_path) {
  if (!file.exists(file_path)) { # Is the path valid?
    stop("File doesn't exist.")
    
  } else { # Is the path a .nc object to read in? 
