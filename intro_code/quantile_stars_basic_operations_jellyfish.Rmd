---
title: "Quantile stars objects for Jellyfish: basic operations"
...
# Replace this with the path to the stars object
stars_path <- "/mnt/ecocast/projectdata/students/ojohnson/copernicus/salpa/versions/salp.0/salp.0.00/preds/daily_resolution/salpa_daily_resolution_monthly.nc"
file.exists(stars_path)
```
...
```{r}
# Read in the data
example_stars <- stars_path |> stars::read_mdim()
...
Stars objects are 3D data cubes with multiple attributes. For our case, each attribute is a quantile percentage of predictions (i.e. 0/50/100 corresponds to min/median/max of predictions across all model folds). Dimensions are specified across longitude, latitude, and month-of-year ("time") using CRS 4326. NA values correspond to land.

* Each month-of-year is represented as the first day of said month-year I.e. July 1999 is "07-01-1999". 
...
example_50perc_Jan1993 <- example_stars["50%",,,1]
...
basic_plot_stars(example_50perc_Jan1993, "January 1993 salpa median patch probability")
```

Use `st_get_dimension_values()` to retrieve the actual values of any specific dimension. 

```{r}
# Examining the first ten month-years in the example stars object
st_get_dimension_values(example_stars, which = "time")[1:10]
```
...
basic_plot_stars(example_stars["max_minus_min",,,1], "January 1993 salpa range in patch probability")
```
...
cropped_stars <- st_crop(example_50perc_Jan1993, 
...
# St_warp coerces one stars object to match another's dimensions/extent
stars_warped <- st_warp(src = example_50perc_Jan1993, dest = stars_transformed_cropped)
...
```{r}
ex_df <- as.data.frame(example_50perc_Jan1993)

head(ex_df)

ex_df$land_mask <- !complete.cases(ex_df)

st_as_stars(ex_df, dims = c("lon", "lat", "time"))
```

More resources about stars: 

* [Merging stars objects](https://tmieno2.github.io/R-as-GIS-for-Economists/merging-stars-objects-using-c-and-st_mosaic.html)
